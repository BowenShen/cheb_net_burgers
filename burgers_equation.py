...
class BurgersPINN(nn.Module):
    def __init__(self):
        ...
        self.net = nn.Sequential(
            nn.Linear(3, 30),  # Input layer (x, y, t) -> 20 neurons
            nn.ReLU(),  # Activation function
            nn.Linear(30, 30),  # Hidden layer with 20 neurons
            nn.ReLU(),  # Activation function
            nn.Linear(30, 30),  # Output layer (u(x, y, t))
            nn.ReLU(),  # Activation function
            nn.Linear(30, 1)  # Output layer (u(x, y, t))
        )
        self.weight_boundary = nn.Parameter(1/3*torch.ones(1))
        self.weight_initial = nn.Parameter(1/3*torch.ones(1))
        self.weight_residual = nn.Parameter(1/3*torch.ones(1))
    def forward(self, x):
        return self.net(x)
